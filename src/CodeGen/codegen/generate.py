...
def getfreereg(instrcution_number,nextuse,preserve_reg):
    ...
    if AddrDesc[variable_name]['memory'] == None:
        ...
        generateHelper.writeInstr("shl $3 , "+L2)
        ...
        #generateHelper.writeInstr("movmov")
...
def getReg(instrcution_number,src1,nextuse):
    ...
    print("hello",src1,AddrDesc[src1]['memory'])
    if reg != None:
        ...
        if AddrDesc[src1]['memory'] == None:
            ...
            if int(shift)>=0:
                ...
                temp = None
                if AddrDesc[src1]['memory']['val'] in AddrDesc:
...
def array_gen(instrcution_number,nextuse,dictionary,preserve_reg,reg):
    ...
    if int(shift)>=0:
        ...
        temp = None
        if dictionary['memory']['val'] in AddrDesc:
    ...
    for i in range(st,end+1):
        ...
        if ir[i].type in type_2:
            ...
            if 'array' in ir[i].src1 and ir[i].src1['array'] == 'True':
                ...
                AddrDesc[name]['memory']['base'] = str(-int(ir[i].src1['addr'])*2)

                if 'const' in ir[i].src1['array_offset']:
                    AddrDesc[name]['memory']['offset'] = str(int(ir[i].src1['array_offset']['val']))    
                else:
                    AddrDesc[name]['memory']['offset'] = str(-int(ir[i].src1['array_offset']['addr'])*2)
                AddrDesc[name]['memory']['val'] = ir[i].src1['array_offset']['val']
                
            if ir[i].dst['name'] in rep and ir[i].src1['name'] in opposite :
                continue
            elif ir[i].dst['name'] in rep and ir[i].src1['name'] not in opposite:
                if(ir[i].src1['type']=='constant'):
                    ...
                    if int(shift)>=0:
                        ...
                        temp = None
                        if AddrDesc[name]['memory']['val'] in AddrDesc:
                    ...
                    if AddrDesc[ir[i].src1['name']]['reg'] != None:
                        ...
                        if type(AddrDesc[ir[i].src1['name']]['memory']) == dict:
                            array_gen(instrcution_number,nextuse,AddrDesc[ir[i].src1['name']],[L,L2,L3],L3)
                        else:
            ...
            name = ir[i].dst['name']
            #print(ir[i].src2['type'],ir[i].src1['type'],ir[i].dst['type'],ir[i].type)
        ...
        if(i==end):
            if ir[i].type in type_1:
                ...
                generateHelper.writeInstr("cmp $0, %rax")
                ...
                if(ir[i].src1['type']=='constant'):
                    ...
                    if(AddrDesc[ir[i].src1['name']]['reg']==None):
                        if type(AddrDesc[ir[i].src1['name']]['memory']) == dict:
                            ...
                            generateHelper.writeInstr("mov "+AddrDesc[ir[i].src1['name']]['memory']+"(%rbp), %rax")
                    else:
                        generateHelper.writeInstr("mov "+AddrDesc[ir[i].src1['name']]['reg']+", %rax")
                saveDirtyAndClean() #clean everything except eax
